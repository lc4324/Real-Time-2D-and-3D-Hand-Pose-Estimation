...
from hand_shape_pose.util.asm_loss import compute_asm_loss
import time
...
def main():
    ...
    for epoch in range(num_epoch):
        ...
        if epoch < num_epoch//5:
          asm_weight = 100
        elif num_epoch//5 <= epoch < 2*num_epoch//5:
          asm_weight = 50
        elif 2*num_epoch//5 <= epoch < 3*num_epoch//5:
          asm_weight = 25
        elif 3*num_epoch//5 <= epoch < 4*num_epoch//5:
          asm_weight = 10
        ...
        for i, batch in enumerate(data_loader_val):
            ...
            asm_loss = compute_asm_loss(est_pose_cam_xyz, gt_pose_cam_xyz, device)
            loss = 0.1*hm_loss(est_hm_list, gt_heatmap) + pose_loss(est_pose_cam_xyz, gt_pose_cam_xyz) + asm_weight*asm_loss

            # backpropagation
            loss.backward()
            optimizer.step()
            total_loss_train += loss.item()
            total_loss_asm += asm_weight*asm_loss.item()

        # record time
        toc = time.time()
        print('loss of epoch %2d: %6.2f, L_asm: %5.2f, time: %0.4f s' %(int(epoch+1), total_loss_train, total_loss_asm, toc-tic))
