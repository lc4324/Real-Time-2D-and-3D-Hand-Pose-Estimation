...
from hand_shape_pose.util.cos_loss_v2 import compute_cos_loss_v2
import time
...
def main():
    ...
    for epoch in range(num_epoch):
        ...
        for i, batch in enumerate(data_loader_val):
            ...
            cos_loss = compute_cos_loss_v2(est_pose_cam_xyz, gt_pose_cam_xyz, device)
            loss = 0.1*hm_loss(est_hm_list, gt_heatmap) + pose_loss(est_pose_cam_xyz, gt_pose_cam_xyz) + 10*cos_loss
        ...
        # record time
        toc = time.time()
        print('loss of epoch %2d: %6.2f, L_cos: %4.2f, time: %0.4f s' %(int(epoch+1), total_loss_train, tot_loss_cos, toc-tic))
